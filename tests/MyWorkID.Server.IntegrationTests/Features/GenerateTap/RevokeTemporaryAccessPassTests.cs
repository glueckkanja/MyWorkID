...
namespace MyWorkID.Server.IntegrationTests.Features.GenerateTap
{
    public class RevokeTemporaryAccessPassTests : IClassFixture<TestApplicationFactory>
    {
        ...
        public RevokeTemporaryAccessPassTests(TestApplicationFactory testApplicationFactory)
        {
            ...
            configuredTestApplicationFactory.AddAuthContextConfig(
                AppFunctions.GenerateTap.ToString(),
                _validAuthContextId
            );
        ...
        public async Task RevokeTemporaryAccessPass_WithoutAuth_Returns401()
        {
            ...
            var response = await unauthenticatedClient.DeleteAsync(
                _baseUrl,
        ...
        {
            var client = TestHelper.CreateClientWithRole(
                _testApplicationFactory,
                provider => provider.WithResetPasswordRole()
            );
        ...
        public async Task RevokeTemporaryAccessPass_WithAuth_WithoutAppSetting_Returns500WithMessage()
        {
            ...
            var client = TestHelper.CreateClientWithRole(
                testApp,
                provider => provider.WithGenerateTapRole()
            );
            var response = await client.DeleteAsync(
                _baseUrl,
                TestContext.Current.CancellationToken
            ...
            problemDetails!
                .Detail.Should()
                .Be(Strings.ERROR_MISSING_OR_INVALID_SETTINGS_GENERATE_TAP);
        }
        ...
        {
            var client = TestHelper.CreateClientWithRole(
                _configuredTestApplicationFactory,
                provider => provider.WithGenerateTapRole()
            );
            var response = await client.DeleteAsync(
                _baseUrl,
            ...
            var client = TestHelper.CreateClientWithRole(
                ...
                provider =>
                    provider
                        .WithGenerateTapRole()
                        .WithRandomSubAndOid()
                        .WithAuthContext(_validAuthContextId)
            );
            var response = await client.DeleteAsync(
                _baseUrl,
                TestContext.Current.CancellationToken
