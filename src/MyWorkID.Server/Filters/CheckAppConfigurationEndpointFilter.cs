...
namespace MyWorkID.Server.Filters
{
    public class CheckAppConfigurationEndpointFilter : IEndpointFilter
    {
        ...
        public CheckAppConfigurationEndpointFilter(
            AppFunctions appFunction,
            IOptions<AppFunctionsOptions> appFunctionsOptions
        )
        {
        ...
        )
        {
            switch (_appFunction)
            {
                case AppFunctions.DismissUserRisk:
                    if (
                        string.IsNullOrEmpty(_appFunctionsOptions.DismissUserRisk)
                        || !AuthContextValidator.IsValidAuthContext(
                            _appFunctionsOptions.DismissUserRisk
                        )
                    )
                    {
                        return Results.Problem(
                            Strings.ERROR_MISSING_OR_INVALID_SETTINGS_DISMISS_USER_RISK,
                ...
                case AppFunctions.GenerateTap:
                    if (
                        string.IsNullOrEmpty(_appFunctionsOptions.GenerateTap)
                        || !AuthContextValidator.IsValidAuthContext(
                            _appFunctionsOptions.GenerateTap
                        )
                    )
                    {
                        return Results.Problem(
                            Strings.ERROR_MISSING_OR_INVALID_SETTINGS_GENERATE_TAP,
                            statusCode: StatusCodes.Status500InternalServerError
                        );
                    }
                    break;
                ...
                case AppFunctions.ResetPassword:
                    if (
                        string.IsNullOrEmpty(_appFunctionsOptions.ResetPassword)
                        || !AuthContextValidator.IsValidAuthContext(
                            _appFunctionsOptions.ResetPassword
                        )
                    )
                    {
                        return Results.Problem(
                            Strings.ERROR_MISSING_OR_INVALID_SETTINGS_RESET_PASSWORD,
                            statusCode: StatusCodes.Status500InternalServerError
                        );
                    }
                    break;
    ...
    {
        public CheckGenerateTapAppConfigurationEndpointFilter(
            IOptions<AppFunctionsOptions> appFunctionsOptions
        )
            : base(AppFunctions.GenerateTap, appFunctionsOptions) { }
    }
    ...
    {
        public CheckResetPasswordAppConfigurationEndpointFilter(
            IOptions<AppFunctionsOptions> appFunctionsOptions
        )
            : base(AppFunctions.ResetPassword, appFunctionsOptions) { }
    }
    ...
    {
        public CheckDismissUserRiskAppConfigurationEndpointFilter(
            IOptions<AppFunctionsOptions> appFunctionsOptions
        )
            : base(AppFunctions.DismissUserRisk, appFunctionsOptions) { }
    }
}
