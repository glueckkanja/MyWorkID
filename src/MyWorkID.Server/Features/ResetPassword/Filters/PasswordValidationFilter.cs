...
namespace MyWorkID.Server.Features.ResetPassword.Filters
{
    ...
    public class PasswordValidationFilter : IEndpointFilter
    {
        ...
        {
            PasswordResetRequest pwRequest = context.GetArgument<PasswordResetRequest>(0);
            ...
            if (pwRequest.NewPassword == null || string.IsNullOrWhiteSpace(pwRequest.NewPassword))
            {
                validationProblemDetails.Errors.Add(
                    nameof(pwRequest.NewPassword),
                    [Strings.PASSWORD_VALIDATION_MISSING_ERROR]
                );
            ...
            if (pwRequest.NewPassword.Length is < 8 or > 255)
            {
                validationProblemDetails.Errors.Add(
                    nameof(pwRequest.NewPassword),
                    [Strings.PASSWORD_VALIDATION_LENGTH_ERROR]
                );
                return Results.ValidationProblem(validationProblemDetails.Errors);
            }
            ...
            counter += patterns.Count(p =>
                Regex.IsMatch(
                    pwRequest.NewPassword,
                    p,
                    RegexOptions.None,
            ...
            if (counter < 3)
            {
                validationProblemDetails.Errors.Add(
                    nameof(pwRequest.NewPassword),
                    [Strings.PASSWORD_VALIDATION_SYMBOLS_ERROR]
                );
                return Results.ValidationProblem(validationProblemDetails.Errors);
            }
